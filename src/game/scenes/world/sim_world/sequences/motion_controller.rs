...
impl MotionController {
    ...
    pub fn current_state(&self, assets: &AssetReader) -> State {
        self.pending
            .back()
            .and_then(|context| assets.get_motion(context.motion_info.motion))
    ...
    pub fn update(&mut self, delta_time: f32, assets: &AssetReader) {
        ...
        if self
            .active
            .as_ref()
            .and_then(|active| assets.get_motion(active.motion_info.motion))
        ...
        if has_pending {
            ...
            if next_is_immediate {
                ...
                if let Some(interrupted) = self.active.as_ref() {
                    self.handle_immediate_interrupt(interrupted, assets);
                }

                if let Some(next) = self.pending.pop_front() {
                    self.promote_to_active(next, assets);
                }
                return;
            }
        ...
        if let Some(active) = self.active.as_mut() {
            if let Some(motion) = assets.get_motion(active.motion_info.motion) {
                let mut terminal_frame_index = None;
                active.current_time_ticks = active.current_time_ticks.saturating_add(delta_time_ms);

                if Self::is_active_motion_finished(active, motion) {
                    let duration_ticks = Self::active_motion_duration_ticks(active, motion);
                    ...
                    if active.transition_guard || active.remaining_repeats > 0 {
                        ...
                        terminal_frame_index = Self::active_terminal_frame_index(active, motion);
                ...
                sampled_root_motion = Some(Self::sample_root_motion_delta(active, motion));
    ...
    fn promote_to_active(&mut self, next: MotionInfoContext, assets: &AssetReader) {
        ...
        if let Some(active) = self.active.as_ref()
            && let Some(motion) = assets.get_motion(active.motion_info.motion)
        {
    ...
    fn handle_immediate_interrupt(&self, interrupted: &ActiveMotionInfo, assets: &AssetReader) {
        if let Some(motion) = assets.get_motion(interrupted.motion_info.motion) {
            tracing::debug!(
    ...
    fn is_active_motion_finished(active: &ActiveMotionInfo, motion: &Motion) -> bool {
        let duration_ticks = Self::active_motion_duration_ticks(active, motion);
        if duration_ticks <= 0 {
            return true;
        }
    ...
    fn active_motion_duration_ticks(active: &ActiveMotionInfo, motion: &Motion) -> i32 {
        let end_frame_count = Self::active_motion_end_frame_count(active, motion);
        active
            .scaled_ticks_per_frame
    ...
    fn active_terminal_frame_index(_active: &ActiveMotionInfo, motion: &Motion) -> Option<u32> {
    ...
    fn active_motion_end_frame_count(_active: &ActiveMotionInfo, motion: &Motion) -> i32 {
        // Original completion checks use frame-count duration (not end-frame-index).
        let mut end_frame_count = motion.frame_count as i32;

        if motion.has_flags(MotionFlags::SKIP_LAST_FRAME) {
    ...
    fn sample_root_motion_delta(active: &mut ActiveMotionInfo, motion: &Motion) -> Vec3 {
        if motion.has_flags(MotionFlags::NO_LVE_MOTION) {
